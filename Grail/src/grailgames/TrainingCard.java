...
public abstract class TrainingCard extends Card{
	...
	/** Reader used to prompt for more information, like dueler */
	 ...
	 * 
	 * @param game
	 *            - the game which cards belong to
	 * @param duelerIndex
	 *            - the index of the dueler in the field which the card is
	 *            acting upon
	 */
		...
		try {
			...
			if (lineUser.equals("")) {
				...
				if (index >= 0 && index < 7) {
					...
					System.out.println(c.currentPlayer.getName() + " used "
							+ getName() + "!");
					actUpon(c, index);
				} else {
					System.out.println("Invalid input!  Please try again:");
